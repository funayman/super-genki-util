...
public class FuriganaView extends View {
  ...
  private void init(Context context, AttributeSet attrs) {
    TypedArray ta = context.getTheme().obtainStyledAttributes(attrs, R.styleable.FuriganaView, 0, 0);
    baseColor = ta.getColor(R.styleable.FuriganaView_baseColor, getResources().getColor(R.color.defaultBaseColor));
    highlightColor = ta.getColor(R.styleable.FuriganaView_highlightColor, getResources().getColor(R.color.defaultHighlightColor));
    furiganaColor = ta.getColor(R.styleable.FuriganaView_furiganaColor, getResources().getColor(R.color.defaultFuriganaColor));
    baseTextSize = ta.getDimensionPixelSize(R.styleable.FuriganaView_baseTextSize, 36);

    // main text
    m_paint_k_norm = new TextPaint();
    m_paint_k_norm.setColor(baseColor);
    m_paint_k_norm.setTextSize(baseTextSize);
    m_paint_k_norm.setTypeface(Typeface.DEFAULT);
    m_paint_k_norm.setFakeBoldText(true);

    // highlighted text
    m_paint_k_mark = new TextPaint();
    m_paint_k_mark.setColor(highlightColor);
    m_paint_k_mark.setTextSize(baseTextSize);
    m_paint_k_mark.setFakeBoldText(true);

    // furigana text
    m_paint_f = new TextPaint();
    m_paint_f.setColor(furiganaColor);
    m_paint_k_norm.setTypeface(Typeface.create(Typeface.DEFAULT, Typeface.NORMAL));
    m_paint_f.setTextSize(m_paint_k_norm.getTextSize() / 2.0f);
  }
